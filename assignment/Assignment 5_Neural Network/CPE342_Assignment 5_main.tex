...
การอัปเดตพารามิเตอร์ $\boldsymbol{\theta} = \{\mathbf{W}, \mathbf{b}\}$ ขึ้นอยู่กับ Optimizer:
\begin{enumerate}[leftmargin=18pt, itemsep=3pt]
    ...
    \item \textbf{Adagrad}: ปรับอัตราการเรียนรู้ตามผลรวมกำลังสองของเกรเดียนต์ในอดีต $G_t = G_{t-1} + g_t^2$, $\boldsymbol{\theta}_{t+1} = \boldsymbol{\theta}_t - \frac{\alpha}{\sqrt{G_t + \epsilon}} g_t$ ทำให้ค่าน้ำหนักที่ถูกอัปเดตบ่อยมีก้าวที่เล็กลง แต่มีข้อจำกัดคือ $G_t$ สะสมต่อเนื่องจนก้าวเล็กลงเรื่อย ๆ จนหยุดเรียนรู้ก่อนเวลา
    \item \textbf{RMSprop}: แก้ไขปัญหาของ Adagrad โดยใช้น้ำหนักถ่วงแบบลดหลั่นทวีคูณ (Exponential Moving Average):
    $s_t = \beta s_{t-1} + (1-\beta) g_t^2$, $\boldsymbol{\theta}_{t+1} = \boldsymbol{\theta}_t - \frac{\alpha}{\sqrt{s_t + \epsilon}} g_t$
