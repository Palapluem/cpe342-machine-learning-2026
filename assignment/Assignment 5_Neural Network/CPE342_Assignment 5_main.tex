...
\clearpage
\section*{5. การตอบคำถามประจำการทดลองและการวินิจฉัยเชิงลึก\\ \large(Experiment Question Solutions \& Rationale)}

\subsection*{คำถามที่ 1: การวิเคราะห์สภาวะ Overfitting ในโมเดลพื้นฐาน\\ \normalsize(Question 1: Baseline DNN Overfitting Analysis)}
...
\vspace{0.4em}
\textbf{คำตอบ (ANSWER)}:
โมเดลพื้นฐาน (Baseline DNN: 1 Hidden Layer 512 Units, SGD $\alpha=0.01$, Batch Size 128) \textbf{ยังไม่เกิดสภาวะ Overfitting ภายใน 10 Epochs ที่ทำการฝึกสอน} โดยมีเหตุผลสนับสนุนเชิงประจักษ์ดังนี้:

\begin{enumerate}[leftmargin=18pt, itemsep=4pt]
    \item \textbf{พฤติกรรมของ Validation Loss (Monotonic Non-Divergence)}:
    สัญญาณทางทฤษฎีที่บ่งชี้ว่าโครงข่ายประสาทเริ่มเกิดสภาวะ Overfitting คือจุดวกกลับที่ \textit{Validation Loss เริ่มปรับตัวสูงขึ้น (Divergence)} ในขณะที่ Training Loss ยังคงลดลงอย่างต่อเนื่อง แต่จากกราฟเส้นทางการเรียนรู้ (Figure 3) พบว่าค่า Training Loss ลดลงจาก $1.875 \rightarrow 0.285$ และ Validation Loss ลดลงตามจาก $1.398 \rightarrow 0.232$ ทั้งสองเส้นทางปรับตัวลดลงอย่างต่อเนื่องและขนานกันไปตลอดทั้ง 10 Epochs โดยไม่มีการวกกลับขึ้นแต่อย่างใด
    \item \textbf{เสถียรภาพของช่องว่างการสรุปนัยทั่วไป (Generalization Gap Stability)}:
    ผลต่างระหว่างความแม่นยำบนชุดตรวจสอบและความแม่นยำบนชุดฝึกสอน ($\Delta = \text{val\_accuracy} - \text{train\_accuracy}$) คงที่ในแดนบวกตลอดการฝึกสอน โดย Validation Accuracy ปรับตัวขึ้นจาก $\approx 64\%$ สู่ $\approx 93.6\%$ ซึ่งสูงกว่า Training Accuracy ($\approx 92.1\%$) เล็กน้อย ซึ่งเป็นลักษณะปกติของ Keras เนื่องจากการวัดผล Validation กระทำหลังจากสิ้นสุดการปรับปรุงค่าน้ำหนักประจำรอบแล้ว
    \item \textbf{สภาวะการเรียนรู้ของโมเดล (Active Convergence Regime)}:
    ภายใต้อัตราการเรียนรู้ระดับต่ำ ($\alpha=0.01$) ร่วมกับ Optimizer แบบ SGD ปกติ จำนวน 10 Epochs (เทียบเท่ากับการอัปเดตน้ำหนักเพียง 4,220 ครั้งบน Batch Size 128) ยังไม่เพียงพอที่จะผลักดันให้พารามิเตอร์ 407,050 ตัวเกิดการท่องจำสัญญาณรบกวนในชุดข้อมูล 54,000 ภาพ โมเดลจึงยังคงอยู่ในระยะ \textbf{Active Convergence / Slight Underfitting} และยังสามารถเรียนรู้เพิ่มเติมเพื่อเพิ่มความแม่นยำได้หากเพิ่มจำนวน Epochs
\end{enumerate}
\end{ansbox}

\clearpage
\subsection*{คำถามที่ 2: การประเมินผลบนชุดทดสอบและการวิเคราะห์ Bias-Variance\\ \normalsize(Question 2: Test Evaluation, Overfitting/Underfitting \& Bias-Variance Diagnostics)}

\begin{ansbox}
\textbf{โจทย์ (Question)}: \textit{Evaluate your model with test set using \texttt{.evaluate()} and compare the results to those from the training and validate sets. Does your model overfit or underfit? How about the bias and variance?}

\vspace{0.4em}
\textbf{คำตอบ (ANSWER)}:
จากการประเมินผลโมเดลพื้นฐานบนชุดข้อมูลทดสอบจริง (Hold-out Test Set, $N=10,000$) ผ่านฟังก์ชัน \texttt{.evaluate()} ได้ผลลัพธ์เชิงเปรียบเทียบและการวินิจฉัยดังนี้:

\begin{enumerate}[leftmargin=18pt, itemsep=3pt]
    \item \textbf{การเปรียบเทียบผลลัพธ์ระหว่าง 3 ส่วนข้อมูล (Dataset Partition Comparison)}:\par
    \vspace{0.25em}
    \noindent\resizebox{\linewidth}{!}{%
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Dataset Partition} & \textbf{จำนวนตัวอย่าง ($N$)} & \textbf{Loss (CCE)} & \textbf{Accuracy (\%)} & \textbf{Generalization Gap} \\
        \midrule
        Training Set & 54,000 & 0.2851 & 92.11\% & Baseline (0.00\%) \\
        Validation Set & 6,000 & 0.2318 & 93.63\% & +1.52\% \\
        Test Set (Unseen) & 10,000 & 0.2657 & 92.41\% & +0.30\% \\
        \bottomrule
    \end{tabular}%
    }\par
    \vspace{0.25em}
    \item \textbf{การวินิจฉัยภาวะ Overfitting หรือ Underfitting}:
    \begin{itemize}[leftmargin=12pt, itemsep=2pt]
        \item \textbf{โมเดลไม่เกิดภาวะ Overfitting}: ค่า Test Accuracy ($92.41\%$) และ Test Loss ($0.2657$) สอดคล้องแนบสนิทกับ Training Accuracy ($92.11\%$) โดยมี Generalization Gap ต่ำกว่า $0.30\%$ แสดงว่าโมเดลไม่มีการท่องจำข้อมูล
        \item \textbf{โมเดลอยู่ในภาวะ Underfitting เล็กน้อย (Mild Underfitting)}: สำหรับงาน MNIST ซึ่งมีระดับความยากไม่สูงและมนุษย์มีความผิดพลาดต่ำกว่า $1\%$ ความผิดพลาดระดับ $\approx 7.6\%$ บนโมเดล 407,050 พารามิเตอร์ สะท้อนว่าโมเดลยังไม่ถูกปลดล็อคศักยภาพสูงสุดใน 10 รอบการฝึกสอน
    \end{itemize}
    \item \textbf{การวิเคราะห์ความเอนเอียงและความแปรปรวน (Bias and Variance Tradeoff)}:
    \begin{itemize}[leftmargin=12pt, itemsep=2pt]
        \item \textbf{Variance (ต่ำมาก - Extremely Low Variance)}: ผลต่างระหว่างข้อผิดพลาดบนชุดฝึกสอน ($7.89\%$) และชุดทดสอบ ($7.59\%$) แทบเป็นศูนย์ บ่งชี้ว่าโมเดลมีความทนทานสูงมากต่อความผันแปรของข้อมูลตัวอย่างใหม่
        \item \textbf{Bias (ปานกลาง - Moderate Bias)}: อัตราข้อผิดพลาดรวม $7.59\%$ บนชุดฝึกสอนเกิดจากอัตราการเรียนรู้คงที่ขนาดเล็กของ SGD ทำให้เกรเดียนต์ยังเคลื่อนตัวไปไม่ถึงจุดต่ำสุดสากล ความเอนเอียงนี้สามารถขจัดได้อย่างง่ายดายเมื่อเปลี่ยนไปใช้ Adaptive Optimizer เช่น Adam หรือ RMSprop (ความแม่นยำพุ่งสู่ $>98\%$)
    \end{itemize}
\end{enumerate}
\end{ansbox}

\clearpage
\subsection*{คำถามที่ 3: การวิเคราะห์ข้อผิดพลาดผ่าน Confusion Matrix\\ \normalsize(Question 3: Confusion Matrix, Misclassification, Precision \& Recall)}
...
\clearpage
\subsection*{คำถามที่ 4: สรุปผลการปรับแต่งไฮเปอร์พารามิเตอร์\\ \normalsize(Question 4: Model Tuning Discussion \& Findings)}
...
รายละเอียดต่อไปนี้คือโค้ด Python ที่ใช้แก้ปัญหาและวิเคราะห์แบบจำลอง Deep Neural Network บนชุดข้อมูล MNIST พร้อมผลลัพธ์และกราฟทั้งหมด ซึ่งได้รันจริงจาก Jupyter Notebook (\texttt{Assignment\_5\_Deep\_Neural\_Network.ipynb} หรือ \texttt{1027\_1042\_1067.ipynb})

\input{notebook_appendix_5.tex}
