...
for i in range(8):
    ax = plt.subplot2grid((3, 8), (0, i)); ax.imshow(raw_imgs[i]); ax.set(title='Cat' if raw_lbls[i] == 0 else 'Dog'); ax.axis('off')
...
augs = [(base, 'Original'), (np.fliplr(base), 'Flip'), (rot_im, 'Rot 15°'), (zm_im, 'Zoom 1.2x'), (np.clip((base - 0.5) * 1.3 + 0.5, 0, 1), 'Contrast')]
...
for k, (lbl, mask) in enumerate([('Cat', raw_lbls == 0), ('Dog', raw_lbls == 1)]):
    ax = plt.subplot2grid((3, 3), (2, k))
    for c_i, c_n in enumerate(['Red', 'Green', 'Blue']):
        ax.hist(raw_imgs[mask][..., c_i].ravel(), bins=bins, alpha=0.6, density=True, label=c_n)
    ax.set(title=f'{lbl} RGB Density'); ax.legend(fontsize=7)
ax = plt.subplot2grid((3, 3), (2, 2))
ax.hist(raw_imgs[raw_lbls == 0].ravel(), bins=bins, alpha=0.5, density=True, label='Cat')
ax.hist(raw_imgs[raw_lbls == 1].ravel(), bins=bins, density=True, alpha=0.5, label='Dog')
ax.set(title='Combined Contrast'); ax.legend(fontsize=7)
plt.suptitle('Figure 1: Samples, Augmentations, and RGB Channels')
