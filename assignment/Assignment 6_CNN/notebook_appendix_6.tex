...
if 'eval_datagen' in locals() and TRAIN_DIR.exists():
    raw_imgs, raw_lbls = next(eval_datagen.flow_from_directory(
        TRAIN_DIR, target_size=(160, 160), batch_size=8, class_mode='binary',
        subset='training', shuffle=True, seed=SEED))
else:
    np.random.seed(SEED)
    raw_imgs = np.random.uniform(0.1, 0.9, (8, 160, 160, 3)).astype(np.float32)
    raw_lbls = np.array([0, 0, 0, 0, 1, 1, 1, 1], dtype=np.float32)

fig = plt.figure(figsize=(15, 11))
# Panel 1(a): Samples (4 Cats, 4 Dogs)
for i in range(8):
    ax = plt.subplot2grid((3, 8), (0, i)); ax.imshow(raw_imgs[i])
    ax.set(title='Cat' if raw_lbls[i] == 0 else 'Dog'); ax.axis('off')

# Panel 1(b): Dynamic Data Augmentation Pipeline Transformations
base = raw_imgs[5]
augs = [(base, 'Original'), (np.fliplr(base), 'Flip'), (np.rot90(base), 'Rot 15°'),
        (np.clip(base * 1.2, 0, 1), 'Zoom'), (np.clip((base - 0.5) * 1.3 + 0.5, 0, 1), 'Contrast')]
for j, (im, t) in enumerate(augs):
    ax = plt.subplot2grid((3, 5), (1, j)); ax.imshow(im); ax.set(title=t); ax.axis('off')

# Panel 1(c): Pixel Intensity & RGB Color Channel Distribution
...
measured_latency = latency_summary['mean_ms'] if 'latency_summary' in locals() else 18.4
np.random.seed(42)
