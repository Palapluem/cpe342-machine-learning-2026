...
\begin{lstlisting}[style=pycode]
# =====================================
# STEP 9: Test with Your Own Image & Human Selfie Experiment
# =====================================
import os
...
def predict_image(image_source, is_selfie=False):
    """
    Infers an input image from local file path or web URL,
    displays the image, and outputs classification result with confidence and AR action.
    Supports local student selfie photo (e.g. selfie.jpg) or online portrait.
    """
    try:
        ...
        print(f"Warning: Could not fetch image ({e}), falling back to placeholder.")
        img = Image.new('RGB', (160, 160), color=(180, 140, 100))
    ...
    filter_act = "Apply Dog Ears & Snout" if label == "Dog" else "Apply Cat Ears & Whiskers"
    
    print(f"Image Source     : {image_source}")
    print(f"Inference Result : {label} (Confidence: {confidence*100:.2f}%)")
    print(f"Filter Action    : {filter_act}")
    if is_selfie:
        print("OOD Note         : Human face is out-of-distribution; binary sigmoid forces projection onto Dog/Cat manifold.")
    print("-" * 60)
    return label, confidence

# Check for student's local selfie file first (e.g. selfie.jpg / selfie.png)
local_selfie = None
for candidate in ['Selfie_Pluem.jpg', 'selfie.jpg', 'selfie.png', 'my_selfie.jpg', 'my_selfie.png']:
    if os.path.exists(candidate):
        local_selfie = candidate
        break

# 1. Test standard cat / dog sample
print("=== Test 1: Standard Dog Photo ===")
predict_image("https://images.unsplash.com/photo-1543466835-00a7907e9de1")

# 2. Test Human Selfie (supports user's local photo or default human selfie portrait)
print("\n=== Test 2: Human Selfie Experiment (Lab Instruction Gimmick) ===")
if local_selfie:
    print(f"Found local selfie file: '{local_selfie}'. Running inference on student's selfie...")
    predict_image(local_selfie, is_selfie=True)
else:
    selfie_url = "https://images.unsplash.com/photo-1534528741775-53994a69daeb"
    print("No local 'selfie.jpg' detected. Testing with standard human portrait selfie...")
    predict_image(selfie_url, is_selfie=True)
\end{lstlisting}

\needspace{5\baselineskip}
\noindent\textbf{\small [Out 18]:}
\begin{lstlisting}[style=output]
=== Test 1: Standard Dog Photo ===
...
=== Test 2: Human Selfie Experiment (Lab Instruction Gimmick) ===
Found local selfie file: 'Selfie_Pluem.jpg'. Running inference on student's selfie...
Image Source     : Selfie_Pluem.jpg
Inference Result : Cat (Confidence: 64.69%)
Filter Action    : Apply Cat Ears & Whiskers
OOD Note         : Human face is out-of-distribution; binary sigmoid forces projection onto Dog/Cat manifold.
------------------------------------------------------------
\end{lstlisting}
