...
\vspace{0.25em}
\textbf{คำตอบ (ANSWER)}:
จากการวิเคราะห์งบประมาณเวลาเชิงสถาปัตยกรรม (Architectural Latency Budget Simulation) อ้างอิงจากผลการทดสอบการอนุมานจริงของโมเดล MobileNetV2 (Phase 2) บน GPU ได้ข้อสรุปดังนี้:
...
\begin{enumerate}[leftmargin=14pt, itemsep=1.5pt, topsep=1pt, parsep=0pt]
    ...
    \begin{itemize}[leftmargin=10pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        \item \textbf{1. Frame Ingestion \& Preprocessing}: เวลาสมมติในการดึงเฟรมและลดขนาดเป็น $160 \times 160$ จัดสรร \textbf{3.2 ms}
        \item \textbf{2. Normalization \& Tensor Formatting}: ปรับสเกลช่วงพิกเซลเป็น $[0, 1]$ บน GPU จัดสรร \textbf{1.1 ms}
        \item \textbf{3. MobileNetV2 Forward Pass}: \textbf{วัดจริงบน GPU ได้ 18.4 ms} ($N=170$ ภาพ, Throughput $54.3\text{ FPS}$)
        \item \textbf{4. Temporal Smoothing (EMA)}: ปรับความเรียบความน่าจะเป็น ($\alpha = 0.70$) จัดสรรเวลา \textbf{0.8 ms}
        \item \textbf{5. AR Filter Overlay \& Rendering}: เรนเดอร์กราฟิกเอฟเฟกต์ทับลงบนเฟรม จัดสรรเวลา \textbf{1.5 ms}
...
MobileNetV2 (Phase 1 Frozen) & 328,193 & 96.31\% & 96.10\% & 96.50\% & +2.83\% & \textcolor{green!60!black}{\textbf{ผ่าน}} \\
MobileNetV2 (w/o Augmentation) & 1,842,433 & 95.12\% & 94.80\% & 95.40\% & +3.98\% & \textcolor{green!60!black}{\textbf{ผ่าน}} \\
MobileNetV2 (Top 20 Layers) & 986,369 & 96.80\% & 96.70\% & 96.90\% & +0.05\% & \textcolor{green!60!black}{\textbf{ผ่าน}} \\
