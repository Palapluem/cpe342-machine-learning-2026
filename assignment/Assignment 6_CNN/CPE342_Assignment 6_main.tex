...
\vspace{0.15em}
\textbf{คำตอบ (ANSWER)}:
จากการประเมินผลบนชุดทดสอบ Hold-out Test Set 4,500 ภาพ (แมว 2,250 ภาพ, สุนัข 2,250 ภาพ) ที่ไม่เคยผ่านการฝึกฝนมาก่อน ได้ผลการวิเคราะห์ความพร้อมเชิงพาณิชย์ดังนี้:
...
\begin{enumerate}[leftmargin=13pt, itemsep=1pt, topsep=0.5pt, parsep=0pt]
    ...
    \begin{itemize}[leftmargin=9pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        ...
        \item \textbf{โมเดล MobileNetV2 Transfer Learning}: \textcolor{accentblue}{\textbf{ผ่านเกณฑ์อย่างสมบูรณ์แบบ (PASS)}} ทะลุเกณฑ์ $95.00\%$ ทั้งสองคลาส:
        \begin{itemize}[leftmargin=9pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
            ...
            \item \textbf{Overall Accuracy}: \textbf{97.47\%} (ถูก 4,386 จาก 4,500 ภาพ) | \textbf{ROC-AUC: 0.9969} | \textbf{PR-AUC: 0.9968}
        \end{itemize}
    \end{itemize}
    \item \textbf{ผลกระทบเชิงปฏิบัติการของความเที่ยงตรง (Operational Impact of Precision)}:
    \begin{itemize}[leftmargin=9pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        \item \textbf{ความสำคัญของการควบคุม False Positive}: ในแอปพลิเคชันเชิงโต้ตอบ เช่น AR Video Filter การเกิด False Positive (เช่น แมวถูกจำแนกเป็นสุนัข แล้วเรนเดอร์กราฟิกหูสุนัขทับ) สร้างความรำคาญใจต่อผู้ใช้สูงสุด
        \item \textbf{อัตราความผิดพลาดต่ำยิ่งยวด}: ด้วย Dog Precision 97.55\% ส่งผลให้อัตรา False Positive Rate (FPR) ต่ำเพียง \textbf{2.45\%} (ผิดพลาดไม่ถึง 3 ครั้งในทุก 100 ครั้ง) รับประกันประสบการณ์ใช้งานที่ดีเยี่ยม
        \item \textbf{ความเสถียรของเกณฑ์การตัดสินใจ}: กราฟ PR Curve รักษาระดับ Precision $> 95\%$ ได้ครอบคลุมตั้งแต่ Recall $0.00$ ถึง $0.98$ ทำให้การใช้ Threshold มาตรฐานที่ $\tau = 0.50$ มีความเสถียรสูง ไม่จำเป็นต้องปรับจูนเพิ่ม
    \end{itemize}
\end{enumerate}
\end{ansbox}
...
\begin{enumerate}[leftmargin=14pt, itemsep=1.5pt, topsep=1pt, parsep=0pt]
    ...
    \begin{itemize}[leftmargin=10pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        \item \textbf{Custom Scratch CNN}: มีพารามิเตอร์สูงถึง \textbf{4,816,513 ตัว} โดยกว่า 55\% กระจุกตัวอยู่ที่ชั้น Dense Classifier หลังจากการ Flatten ($18 \times 18 \times 128 = 41,472 \to 64$ โหนด) ก่อให้เกิด Dense Bottleneck ขนาดมหาศาลและ Overfitting อย่างรวดเร็ว
        \item \textbf{MobileNetV2}: มีพารามิเตอร์เพียง \textbf{2,586,177 ตัว (ลดลง 46.3\%)} โดยใช้ \textbf{Global Average Pooling (GAP)} ยุบมิติเชิงพื้นที่ $5 \times 5 \times 1280$ เหลือเวกเตอร์ 1,280 โดยตรง ขจัดค่าน้ำหนักส่วนเกินและป้องกัน Overfitting ได้อย่างยอดเยี่ยม
    \end{itemize}
    \item \textbf{บทบาทของ Pre-trained Visual Priors}:
    การฝึกฝนจากศูนย์บนภาพ 20,000 ภาพไม่เพียงพอที่โมเดลจะสร้างตัวกรองฟีเจอร์ระดับสูงได้อย่างสมบูรณ์ ในขณะที่ MobileNetV2 ผ่านการเรียนรู้ล่วงหน้าบน ImageNet (1.4 ล้านภาพ, 1,000 คลาส) จึงมีฟิลเตอร์ตรวจจับขอบ (Edges), ลวดลายขน (Textures) และโครงสร้างใบหน้า (Semantic Gestalt) พร้อมใช้งานทันที
    ...
    \begin{itemize}[leftmargin=10pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        ...
        \item \textit{Phase 2 (Fine-Tuning)}: ปลดล็อค 40 เลเยอร์บนสุดร่วมกับอัตราการเรียนรู้ต่ำมาก ($\eta = 10^{-5}$) ช่วยให้ฟิลเตอร์ระดับลึกปรับเข้ากับกายวิภาคของสุนัขและแมวได้อย่างแม่นยำ ปราศจากปัญหา Catastrophic Forgetting
    \end{itemize}
\end{enumerate}
\end{ansbox}
...
\begin{enumerate}[leftmargin=14pt, itemsep=1.5pt, topsep=1pt, parsep=0pt]
    ...
    \begin{itemize}[leftmargin=10pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        ...
        \item \textbf{Model 3 (เปิดใช้งาน Dynamic Augmentation)}: Training Accuracy อยู่ที่ \textbf{97.23\%} และ Validation Accuracy อยู่ที่ \textbf{97.47\%} ส่งผลให้ Generalization Gap ลดลงเหลือเพียง \textbf{+0.24\%} (Overfitting ถูกกำจัดอย่างสมบูรณ์)
    \end{itemize}
...
รายละเอียดต่อไปนี้คือโค้ด Python ที่ใช้แก้ปัญหาและวิเคราะห์แบบจำลอง Convolutional Neural Network (CNN) และการเรียนรู้แบบถ่ายโอน (Transfer Learning) บนชุดข้อมูล Dogs vs. Cats พร้อมผลลัพธ์และกราฟทั้งหมด ซึ่งได้รันจริงจาก Jupyter Notebook (\texttt{CNN\_Homework.ipynb} หรือ \texttt{67\_1027\_1042.ipynb})
