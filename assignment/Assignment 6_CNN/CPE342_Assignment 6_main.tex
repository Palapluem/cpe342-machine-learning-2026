...
การประมวลผลข้อมูลภาพเชิงภาพดิจิทัล (Digital Image Processing) ในยุคก่อนหน้าประสบข้อจำกัดอย่างรุนแรงเมื่อใช้โมเดล Multi-Layer Perceptron (MLP) แบบดั้งเดิม เนื่องจาก MLP มองภาพเป็นเวกเตอร์ 1 มิติที่เรียงต่อกัน ทำให้สูญเสียข้อมูลโครงสร้างเชิงพื้นที่ (Spatial Topology) และการเชื่อมต่อหนาแน่น (Fully-Connected) ก่อให้เกิดพารามิเตอร์มหาศาล ($O(H \times W \times C \times N)$) ซึ่งเสี่ยงต่อการเกิด Overfitting อย่างยิ่ง โครงข่ายประสาทคอนโวลูชัน (Convolutional Neural Networks: CNN) จึงถูกพัฒนาขึ้นเพื่อแก้ไขปัญหานี้โดยเลียนแบบกลไกการมองเห็นของสิ่งมีชีวิต
...
\begin{enumerate}[leftmargin=1.8em, itemsep=1pt]
    \item \textbf{Simple Cells:} ตอบสนองต่อเส้นตรงและขอบภาพ (Edges/Bars) ในทิศทาง (Orientation) และตำแหน่งเฉพาะ
...
\item \textbf{Value Normalization:} ปรับสเกลค่าพิกเซลด้วย $1/255$ เพื่อให้ทุกโมเดลรับข้อมูลในช่วง $[0,1]$ สอดคล้องกัน
\item \textbf{Training-only Augmentation:} ใช้การกลับภาพแนวนอน (Horizontal Flip), การหมุนภาพ ($\pm 15^\circ$), การเลื่อนตำแหน่ง ($\pm 10\%$), การซูม ($\pm 20\%$) และการปรับความสว่างช่วง $0.8$--$1.2$ สำหรับชุดฝึกฝนเท่านั้น ส่วนชุด Validation และ Final Test ใช้เพียงการปรับขนาดและปรับสเกลพิกเซล
\end{enumerate}
...
\vspace{1em}
\clearpage
\section*{3. ระเบียบวิธีวิจัยและสถาปัตยกรรมโมเดล\newline \large(Methodology \& Model Architectures)}
...
% =================================================================
\vspace{1.2em}
\section*{4. ผลการทดลองและการวินิจฉัยโมเดลเชิงลึก\newline \large(Experimental Results \& Diagnostic Plots)}
...
การประเมินผลเชิงลึกบนชุดข้อมูลตรวจสอบ 4,500 ภาพแบบไม่สับเปลี่ยน (Unshuffled Holdout Validation Set) แสดงรายละเอียดดังรูปที่ \ref{fig:plot_4}:
...
จากตารางความถูกต้อง (Confusion Matrix) ในรูปที่ \ref{fig:plot_4}(a) บนชุดตรวจสอบ 4,500 ภาพ:
\begin{itemize}[leftmargin=1.8em, itemsep=1pt]
    ...
    \item \textbf{Overall Validation Accuracy:} \textbf{97.47\%} (Macro F1 = 0.9747, Validation Loss = 0.0682 บนชุดตรวจสอบ 4,500 ภาพ)
\end{itemize}
สำหรับชุดทดสอบขั้นสุดท้ายที่ไม่เคยผ่านการฝึกมาก่อน (Final Test Set, $N = 2,500$ ภาพ) โมเดลทำค่า Test Accuracy ได้ \textbf{97.47\%} และ Test Loss เท่ากับ \textbf{0.0680} ซึ่งสอดคล้องอย่างสมบูรณ์กับผลการตรวจสอบ
...
เราได้นำผลลัพธ์จากชุดการทดลองตัดทอนตัวแปร (Ablation Study) 5 รูปแบบ (โดยโมเดล M1--M3 เป็นโมเดลที่ดำเนินการฝึกฝนและประเมินผลโดยตรงใน Notebook ส่วนโมเดล M4--M5 เป็น Pre-computed Baseline จากการทดลอง Extended Ablation ใน \codevar{benchmark\_results.json}) เพื่อจำแนกผลกระทบของแต่ละองค์ประกอบ:

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{plot_6_ablation_augmentation_finetuning.pdf}
    \caption{การทดลองตัดทอนตัวแปร (Ablation Study): (a) เปรียบเทียบความแม่นยำและความเที่ยงตรง (M1--M3: รันจริงใน Notebook, M4--M5: Pre-computed Baseline) (b) การวิเคราะห์ Generalization Gap ($|\text{Train} - \text{Val}|$\%)}
    \label{fig:plot_6}
\end{figure}

ข้อค้นพบสำคัญจากรูปที่ \ref{fig:plot_6}:
\begin{itemize}[leftmargin=1.8em, itemsep=1pt]
    \item \textbf{กรณีไม่ใช้ Data Augmentation (Model 4):} แม้จะ Fine-Tune โมเดลจนได้ค่า Train Accuracy สูงถึง 99.10\% ทว่า Validation Accuracy ลดลงมาอยู่ที่ 95.12\% ก่อให้เกิด Generalization Gap สูงถึง \textbf{+3.98\%} เป็นหลักฐานยืนยันว่า Data Augmentation มีบทบาทสำคัญยิ่งในการป้องกันการ Overfitting
...
\clearpage
\subsection*{4.5 การจำลองงบประมาณเวลาเชิงสถาปัตยกรรมสำหรับสตรีมมิ่งวิดีโอ\\ \normalsize(Architectural Latency Budget Simulation for Streaming Pipeline)}
เพื่อตอบสนองโจทย์ทางธุรกิจในการนำโมเดลไปต่อยอดสู่แอปพลิเคชัน AR Video Filter เราได้ทำการจำลองงบประมาณเวลาเชิงสถาปัตยกรรม (Architectural Latency Budget\allowbreak{} Simulation) บนสมมติฐานสตรีมมิ่งวิดีโอ 30 FPS ควบคู่กับการทำ Temporal Exponential\allowbreak{} Smoothing ดังแสดงในรูปที่ \ref{fig:plot_7}:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{plot_7_inference_streaming_pipeline.pdf}
    \vspace{-0.4em}
    \caption{การจำลองงบประมาณเวลาเชิงสถาปัตยกรรมสำหรับระบบสตรีมมิ่งวิดีโอ: (a) เส้นเวลาความน่าจะเป็นและการสลับโหมดฟิลเตอร์ด้วย Temporal Exponential Smoothing ($\alpha = 0.70$) (b) การจัดสรรงบประมาณเวลาประมวลผลเชิงวิศวกรรม (Simulated Engineering Budget) รวม 25.0 ms/เฟรม เทียบกับขีดจำกัด 33.33 ms}
    \label{fig:plot_7}
\end{figure}
\vspace{-0.3em}

จากการวิเคราะห์ภาพรวมงบประมาณเวลาประมวลผลในรูปที่ \ref{fig:plot_7}(a)--(b) มีรายละเอียดเชิงวิศวกรรมของท่อส่งข้อมูลดังต่อไปนี้:
\begin{itemize}[leftmargin=1.8em, itemsep=0.5pt, parsep=0pt]
    \item \textbf{เวลาประมวลผลโมเดลที่วัดได้จริง (Measured Pass):} MobileNetV2 Forward Pass บน Apple Silicon macOS arm64 (Metal/MPS Acceleration) วัดจากการรันภาพทดสอบต่อเนื่องแบบ Offline ($N=170$ ภาพ) ได้ค่าเฉลี่ย \textbf{18.4 ms} ต่อภาพ (มัธยฐาน 18.2 ms, P95 20.8 ms, Throughput $54.3\text{ FPS}$) มี Safety Margin ต่ำกว่าเพดาน 33.33 ms อยู่ถึง \textbf{14.93 ms}
    \item \textbf{การจัดสรรเวลาระบบจำลอง (Simulated Allowances):} สำหรับระบบ AR เชิงโต้ตอบ ได้จำลองกรอบเวลาทางวิศวกรรม (Engineering Design Allowances) สำหรับโมดูลอื่น ๆ ได้แก่ Frame Capture \& Downsample (3.2 ms), Tensor Prep \& Normalization (1.1 ms), อนุมานโมเดล MobileNetV2 (18.4 ms วัดจริง), Softmax \& EMA Smoothing (0.8 ms), และ Graphic Overlay (1.5 ms)
    \item \textbf{เวลารวมและข้อสรุปความเป็นไปได้ (Architectural Feasibility):} เวลารวมตามงบประมาณอยู่ที่ \textbf{25.0 ms ต่อเฟรม} (เทียบเท่า Throughput $40.0\text{ FPS}$) มี Safety Margin สูงถึง \textbf{8.33 ms (25.0\%)} ต่ำกว่าเพดาน 33.33 ms ยืนยันความเป็นไปได้ในเชิงสถาปัตยกรรม ทั้งนี้เป็นการประเมินเชิงงบประมาณเวลาจากการทำ Offline Sequential Inference โดยไม่ได้ต่อกล้องเว็บแคมหรือเรนเดอร์กราฟิกสดบนหน้าจอจริง
\end{itemize}
...
เพื่อประเมินพฤติกรรมการตัดสินใจของโมเดลบนภาพถ่ายจริงในสภาพแวดล้อมที่ท้าทาย เราได้ทำการสุ่มตัวอย่างภาพจากชุดข้อมูลตรวจสอบ (Validation Holdout Set) 4,500 ภาพ ทั้งกลุ่มที่โมเดลทำนายได้อย่างแม่นยำสูง และกลุ่มกรณีศึกษาขอบเขตที่เกิดความผิดพลาด (Failure Topology) ดังแสดงในรูปที่ \ref{fig:plot_8}:
...
\begin{itemize}[leftmargin=1.6em, itemsep=0.5pt, topsep=1pt, parsep=0pt]
    ...
    \item \textbf{กรณีท้าทายที่ทำนายถูกต้อง (Hard Invariants):} โมเดลยังคงจำแนกได้ถูกต้องในภาพย้อนแสงจ้า (Backlit\allowbreak{} Silhouette) และภาพที่มีสิ่งกีดขวางบดบังบางส่วน (Partial\allowbreak{} Occlusion) ซึ่งเป็นผลจากการทำ Two-Phase Transfer Learning ร่วมกับ Data Augmentation
\end{itemize}

% =================================================================
\vspace{0.5em}
\clearpage
\section*{5. การตอบคำถามประจำการทดลองและการอภิปรายเชิงวิศวกรรม\newline \large(Experiment Question Solutions \& Engineering Insights)}
...
\begin{enumerate}[leftmargin=13pt, itemsep=1pt, topsep=0.5pt, parsep=0pt]
    ...
    \begin{itemize}[leftmargin=9pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        ...
        \item \textbf{โมเดล Custom Scratch CNN (Baseline เพื่อการเปรียบเทียบ)}: ทำ Dog Precision ได้ \textbf{83.50\%} (ต่ำกว่าเกณฑ์ขั้นต่ำ 90\% ตามคาดหมาย เนื่องจากโครงข่ายเริ่มฝึกจากน้ำหนักสุ่มโดยไม่มี Prior Knowledge)
        \item \textbf{โมเดลหลักเพื่อการปล่อยผลิตภัณฑ์ M3 MobileNetV2}: \textcolor{green!60!black}{\textbf{ผ่านเกณฑ์อย่างสมบูรณ์แบบ (PASS)}} ด้วย Precision สูงกว่า $95.00\%$ ทั้งสองคลาส:
        \begin{itemize}[leftmargin=9pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
            \item \textbf{Dog Precision}: \textbf{97.55\% (0.98)} (ผ่านเกณฑ์ขั้นต่ำ $+7.55\%$ และเกณฑ์อุตสาหกรรม $+2.55\%$) 
    ...
    \begin{itemize}[leftmargin=9pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        \item \textbf{การควบคุม False Discovery Rate}: ด้วยค่า Dog Precision 97.55\% ส่งผลให้อัตรา False Discovery Rate ($\text{FDR} = 1 - \text{Precision}$) ต่ำเพียง \textbf{2.45\%} (สอดคล้องกับ False Positive Rate: $\text{FPR} = \text{FP}/(\text{FP}+\text{TN}) = 55/2,250 = 2.44\%$) ช่วยจำกัดข้อผิดพลาดในการตรวจจับเชิงรุกและเพิ่มความน่าเชื่อถือของระบบในเชิงปฏิบัติการ
...
{\raggedright \textbf{โจทย์ (Question)}: \textit{Compare the performance between your custom CNN trained from scratch and the pre-trained Transfer Learning model. Why does Transfer Learning perform significantly better on this dataset?}\par}
...
\begin{enumerate}[leftmargin=14pt, itemsep=1.5pt, topsep=1pt, parsep=0pt]
    ...
    \begin{itemize}[leftmargin=10pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        \item \textbf{ความแม่นยำ (Accuracy)}: โมเดล MobileNetV2 ทำได้ \textbf{97.47\%} สูงกว่า Custom Scratch CNN (\textbf{84.15\%}) ถึง \textbf{+13.32\%}
        ...
        \item \textbf{พื้นที่ใต้เส้นโค้ง ROC (ROC-AUC)}: MobileNetV2 บรรลุ \textbf{0.9958} เทียบกับ Custom Scratch ที่ทำได้เพียง \textbf{0.9124}
    \end{itemize}
...
\vspace{0.25em}
\textbf{คำตอบ (ANSWER)}:
จากการวิเคราะห์งบประมาณเวลาเชิงสถาปัตยกรรม (Architectural Latency Budget Simulation) อ้างอิงจากผลทดสอบการอนุมานแบบออฟไลน์ (Offline Sequential Inference) ของโมเดล MobileNetV2 (Phase 2) บนชิป Apple Silicon macOS arm64 (MPS Acceleration) ร่วมกับกรอบเวลาจำลองทางวิศวกรรม ได้ข้อสรุปดังนี้:
...
\begin{enumerate}[leftmargin=14pt, itemsep=1.5pt, topsep=1pt, parsep=0pt]
    ...
    \item \textbf{การแจกแจงงบประมาณเวลาเชิงสถาปัตยกรรม (Architectural Latency Breakdown)}:
    \begin{itemize}[leftmargin=10pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        \item \textbf{1. Frame Ingestion \& Preprocessing}: เวลาจำลองในการดึงเฟรมและลดขนาดเป็น $160 \times 160$ (จัดสรรตามกรอบจำลองเชิงวิศวกรรม) \textbf{3.2 ms}
        \item \textbf{2. Normalization \& Tensor Formatting}: ปรับสเกลช่วงพิกเซลเป็น $[0, 1]$ (จัดสรรตามกรอบจำลองเชิงวิศวกรรม) \textbf{1.1 ms}
        \item \textbf{3. MobileNetV2 Forward Pass}: \textbf{วัดจริงแบบ Offline บน Apple Silicon arm64 ได้ 18.4 ms} (มัธยฐาน 18.2 ms, P95 20.8 ms, $N=170$ ภาพ, Throughput $54.3\text{ FPS}$)
        \item \textbf{4. Temporal Smoothing (EMA)}: ปรับความเรียบความน่าจะเป็น ($\alpha = 0.70$, จัดสรรตามกรอบจำลองเชิงวิศวกรรม) \textbf{0.8 ms}
        \item \textbf{5. AR Filter Overlay \& Rendering}: เรนเดอร์กราฟิกเอฟเฟกต์ทับลงบนเฟรม (จัดสรรตามกรอบจำลองเชิงวิศวกรรม) \textbf{1.5 ms}
        \item \textbf{เวลารวมตามงบประมาณ (Total Latency)}: \textbf{25.0 ms ต่อเฟรม} (เทียบเท่า Throughput รวม \textbf{40.0 FPS})
    \end{itemize}
    \item \textbf{ข้อสรุปความเป็นไปได้เชิงสถาปัตยกรรม (Architectural Feasibility)}:
    ระบบจำลองมีส่วนเผื่อเวลาด้านความปลอดภัย (Safety Margin) สูงถึง \textbf{8.33 ms (25.0\%)} ต่ำกว่าเพดาน 33.33 ms ยืนยันความเป็นไปได้ในเชิงสถาปัตยกรรม ทั้งนี้ เป็นการวิเคราะห์งบประมาณเวลาจากผล Offline Inference โดยไม่มีการต่อกล้องหรือเรนเดอร์กราฟิกสด
    \item \textbf{ข้อเสนอแนะในการต่อยอดสู่ Edge / Mobile Devices}:
    แนะนำให้แปลงโมเดลเป็น TensorFlow Lite (TFLite) ร่วมกับการทำ Post-Training Quantization (INT8) เพื่อลดขนาดหน่วยความจำและเร่งความเร็วบนตัวเร่งฮาร์ดแวร์ NPU ของสมาร์ตโฟนสำหรับรองรับการใช้งาน On-Device ในอนาคต
\end{enumerate}
\end{ansbox}

\section*{6. การอภิปรายผลเชิงวิชาการและการเปรียบเทียบ\newline \large(Academic Discussion \& Comparative Synthesis)}
...
\midrule
M1: Custom CNN (Scratch Baseline) & 6,944,449 & 84.15\% & 83.50\% & 85.20\% & +4.55\% & \textcolor{red}{\textbf{ไม่ผ่าน}} \\
M2: MobileNetV2 (Phase 1 Frozen) & 328,193 & 96.31\% & 96.10\% & 96.50\% & +2.83\% & \textcolor{green!60!black}{\textbf{ผ่าน}} \\
M3: \textbf{MobileNetV2 Final (Top 40 FT + Aug)} & \textbf{1,842,433} & \textbf{97.47\%} & \textbf{97.55\%} & \textbf{97.38\%} & \textbf{+0.24\%} & \textcolor{green!60!black}{\textbf{ผ่าน}} \\
M4: MobileNetV2 (w/o Augmentation)* & 1,842,433 & 95.12\% & 94.80\% & 95.40\% & +3.98\% & \textcolor{green!60!black}{\textbf{ผ่าน}} \\
M5: MobileNetV2 (Top 20 Layers FT)* & 986,369 & 96.80\% & 96.70\% & 96.90\% & +0.05\% & \textcolor{green!60!black}{\textbf{ผ่าน}} \\
\bottomrule
\end{tabular}%
}
\vspace{0.3em}
\begin{minipage}{\textwidth}
\footnotesize \textit{*หมายเหตุ: โมเดล M1, M2, M3 ได้รับการดำเนินการฝึกฝนและประเมินผลโดยตรงใน Notebook ส่วนโมเดล M4 และ M5 เป็นชุดข้อมูลผลลัพธ์การทดลองพื้นฐาน (Pre-computed Baseline) จากการศึกษา Extended Ablation ใน benchmark\_results.json}
\end{minipage}
\end{table}
...
\begin{itemize}[leftmargin=1.8em, itemsep=1pt]
    ...
    \item \textbf{MobileNetV2 Phase 1 (Frozen Base):} มี Low Variance อย่างยิ่งเนื่องจากพารามิเตอร์ส่วนใหญ่ถูกแช่แข็ง (Gap 2.83 จุดร้อยละ) แต่ยังมี Inductive Bias เล็กน้อยจากโดเมน ImageNet ทำให้ขีดความสามารถหยุดอยู่ที่ 96.31\%
    \item \textbf{MobileNetV2 Phase 2 (Fine-Tuning + Augmentation):} ให้ผลลัพธ์ที่ดีที่สุดในการทดลองนี้ โดยการปลดล็อค 40 เลเยอร์ช่วยลด Bias ขณะที่ Data Augmentation และ L2 Regularization ช่วยจำกัด Variance (Gap 0.24 จุดร้อยละ)
\end{itemize}
...
\begin{enumerate}[leftmargin=1.8em, itemsep=0.5pt, topsep=1pt, parsep=0pt]
...
\subsection*{6.4 แนวทางการนำโมเดลไปประยุกต์ใช้งานบนระบบ Edge / Mobile Devices\\ \normalsize(Edge Deployment \& Model Quantization Roadmap)}
สถาปัตยกรรม MobileNetV2 ที่พัฒนาขึ้นนี้มีความพร้อมสำหรับการนำไปประยุกต์ใช้งานจริงในอนาคต:
\begin{itemize}[leftmargin=1.8em, itemsep=0.5pt, topsep=1pt, parsep=0pt]
    \item \textbf{TensorFlow Lite (TFLite) Conversion:} รองรับการแปลงโมเดลไปสู่รูปแบบ \texttt{.tflite} เพื่อนำไปประมวลผล On-Device บนสมาร์ตโฟน
    \item \textbf{Post-Training Quantization (INT8):} แนวทางการลดทอนค่าน้ำหนักจาก Float32 เป็น Int8 เป็นการประมาณการเชิงทฤษฎีเพื่อลดการใช้หน่วยความจำและเร่งความเร็วบน NPU สำหรับการใช้งานจริง (โครงงานนี้ประเมินเฉพาะโมเดล Float32 ตามผลทดลองจริง)
\end{itemize}

% =================================================================
\vspace{0.2em}
\section*{7. สรุปผลการทดลองและข้อเสนอแนะเชิงประยุกต์\newline \large(Conclusions \& Practical Recommendations)}
...
\begin{enumerate}[leftmargin=1.8em, itemsep=0.5pt, topsep=1pt, parsep=0pt]
    ...
    \begin{itemize}[leftmargin=1.5em, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
        ...
        \item นำเทคนิค Quantization (INT8) มาประยุกต์ใช้เพื่อรองรับการประมวลผลบนแอปพลิเคชันสมาร์ตโฟน iOS หรือ Android ในรูปแบบ On-\allowbreak{}Device โดยไม่ต้องพึ่งพาคลาวด์
    \end{itemize}
\end{enumerate}
