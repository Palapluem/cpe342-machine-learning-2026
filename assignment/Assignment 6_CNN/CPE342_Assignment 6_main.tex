...
\begin{itemize}[leftmargin=1.8em, itemsep=3pt]
    \item \textbf{EarlyStopping:} หยุดการเทรนหาก Validation Accuracy ไม่พัฒนาต่อเนื่อง 3 รอบ พร้อมคืนค่าน้ำหนักที่ดีที่สุด (\texttt{restore\_best\_weights = True}) ป้องกันการจดจำชุดข้อมูลฝึกสอน
...
\begin{ansbox}
\textbf{โจทย์ (Question)}: \textit{Assume that you work as an ML engineer for a company want to get more attraction on your product by launching a new product that can classify whether it is a dog or a cat. You have decide that you want a precision more than 90 - 95\% in order to launch the product. Explain your evaluation results and why this model meets or fails the business criteria.}

\vspace{0.4em}
\textbf{คำตอบ (ANSWER)}:
จากการประเมินผลบนชุดข้อมูลทดสอบแบบ Hold-out Test Set จำนวน 4,500 ภาพ (2,250 แมว, 2,250 สุนัข) ที่ไม่เคยผ่านการฝึกฝนมาก่อน ได้ผลลัพธ์เชิงประจักษ์และการวิเคราะห์ความพร้อมเชิงพาณิชย์ดังนี้:

\begin{enumerate}[leftmargin=18pt, itemsep=3pt]
    \item \textbf{ผลการประเมินเทียบกับเกณฑ์ทางธุรกิจ (Business Evaluation Benchmark)}:
    \begin{itemize}
        \item \textbf{เกณฑ์ขั้นต่ำของบริษัท (Minimum Baseline)}: Precision $> 90.00\%$
        \item \textbf{เกณฑ์มาตรฐานระดับอุตสาหกรรม (Production Standard)}: Precision $> 95.00\%$
        \item \textbf{โมเดล Custom Scratch CNN}: \textcolor{red}{\textbf{ล้มเหลว (FAIL)}} ไม่ผ่านเกณฑ์ขั้นต่ำ โดยได้ Dog Precision เพียง \textbf{83.50\%} (Cat Precision = 84.80\%, Test Accuracy = 84.15\%) เนื่องจากข้อจำกัดด้านความจุและการเกิด Overfitting บนข้อมูล 20,000 ภาพ
        \item \textbf{โมเดล MobileNetV2 Two-Phase Transfer Learning}: \textcolor{accentblue}{\textbf{ผ่านเกณฑ์อย่างสมบูรณ์แบบ (PASS)}} ทะลุเกณฑ์อุตสาหกรรม $95.00\%$ ทั้งสองคลาส:
        \begin{itemize}
            \item \textbf{Dog Precision (สุนัข)}: \textbf{97.55\% (0.98)} — สูงกว่าเกณฑ์ขั้นต่ำ $+7.55\%$ และสูงกว่าเกณฑ์อุตสาหกรรม $+2.55\%$
            \item \textbf{Cat Precision (แมว)}: \textbf{97.38\% (0.97)} — สูงกว่าเกณฑ์ขั้นต่ำ $+7.38\%$ และสูงกว่าเกณฑ์อุตสาหกรรม $+2.38\%$
            \item \textbf{Overall Test Accuracy}: \textbf{97.47\%} (ทำนายถูกต้อง 4,386 ภาพ จาก 4,500 ภาพ)
            \item \textbf{ROC-AUC: 0.9969} และ \textbf{PR-AUC: 0.9968} สะท้อนถึงความสามารถในการแยกแยะคลาสที่มีความมั่นคงสูงสุด
        \end{itemize}
    \end{itemize}
    \item \textbf{ผลกระทบเชิงปฏิบัติการของความเที่ยงตรง (Operational Impact of Precision)}:
    \begin{itemize}
        \item \textbf{ความสำคัญของ False Positive Rate}: ในแอปพลิเคชันเชิงโต้ตอบ เช่น AR Video Filter หรือระบบกล้องสตรีมมิ่ง การเกิด False Positive (เช่น ใบหน้าแมวถูกจำแนกผิดเป็นสุนัข แล้วระบบเรนเดอร์ฟิลเตอร์หูสุนัขทับลงไป) ส่งผลเสียต่อประสบการณ์ของผู้ใช้อย่างรุนแรง
        \item \textbf{อัตราความผิดพลาดต่ำเป็นพิเศษ}: ด้วย Dog Precision 97.55\% ทำให้อัตราผลบวกลวง (FPR) ต่ำกว่า \textbf{2.45\%} นั่นคือในทุก 100 ครั้งที่มีการตรวจจับและแปะฟิลเตอร์ จะมีความผิดพลาดไม่ถึง 3 ครั้ง รับประกันความพึงพอใจสูงสุดของผู้ใช้งาน
        \item \textbf{ความเสถียรของจุดตัดการตัดสินใจ}: กราฟ Precision-Recall Curve (AUC = 0.9968) แสดงให้เห็นว่าโมเดลรักษาระดับ Precision $> 95\%$ ได้ครอบคลุมตั้งแต่ค่า Recall $0.00$ ถึง $0.98$ ทำให้การใช้ Decision Threshold มาตรฐานที่ $\tau = 0.50$ มีความเสถียรโดยไม่ต้องจูนจุดตัดให้ซับซ้อน
    \end{itemize}
\end{enumerate}
\end{ansbox}
...
\begin{enumerate}[leftmargin=18pt, itemsep=3pt]
    ...
    \begin{itemize}
        ...
        \item \textbf{ความเที่ยงตรงของสุนัข (Dog Precision)}: MobileNetV2 ทำได้ \textbf{97.55\%} สูงกว่า Custom Scratch CNN (\textbf{83.50\%}) ถึง \textbf{+14.05\%}
    ...
    \begin{itemize}
        \item \textbf{Custom Scratch CNN}: มีขนาดพารามิเตอร์สูงถึง \textbf{4,816,513 ตัว} โดยกว่า 55\% กระจุกตัวอยู่ที่ชั้น Dense Classifier หลังจากการ Flatten ($18 \times 18 \times 128 = 41,472 \to 64$ โหนด) ทำให้เกิด Dense Bottleneck ขนาดมหาศาล และเกิด Overfitting อย่างรวดเร็ว
        \item \textbf{MobileNetV2}: มีพารามิเตอร์เพียง \textbf{2,586,177 ตัว (ลดลง 46.3\%)} โดยแทนที่การ Flatten ด้วย \textbf{Global Average Pooling (GAP)} ซึ่งยุบมิติเชิงพื้นที่ $5 \times 5 \times 1280$ เหลือเวกเตอร์ 1,280 โดยตรง ขจัดค่าน้ำหนักส่วนเกินและป้องกัน Overfitting ได้อย่างยอดเยี่ยม
    \end{itemize}
    \item \textbf{บทบาทของ Pre-trained Visual Priors}:
    การฝึกฝนจากศูนย์บนภาพ 20,000 ภาพไม่เพียงพอที่โมเดลจะเรียนรู้ฟิลเตอร์สกัดฟีเจอร์ระดับสูงได้อย่างสมบูรณ์ ในขณะที่ MobileNetV2 ผ่านการเรียนรู้ล่วงหน้าบน ImageNet (1.4 ล้านภาพ, 1,000 คลาส) ทำให้มีตัวกรองขอบ (Edges), ลวดลายเส้นขน (Textures) และโครงสร้างใบหน้า (Semantic Gestalt) พร้อมใช้งานทันที
    \item \textbf{กลยุทธ์การฝึกฝนแบบสองขั้นตอน (Two-Phase Protocol Dynamics)}:
    \begin{itemize}
        \item \textit{Phase 1 (Feature Extraction)}: แช่แข็ง Backbone เทรนเฉพาะส่วนหัวที่ $\eta = 10^{-4}$ เพื่อสร้างค่าน้ำหนักเริ่มต้นของ Classifier โดยไม่ทำลายน้ำหนักเดิมที่เสถียร
        \item \textit{Phase 2 (Fine-Tuning)}: ปลดล็อค 40 เลเยอร์บนสุดร่วมกับอัตราการเรียนรู้ต่ำมาก ($\eta = 10^{-5}$) ช่วยให้ฟิลเตอร์ระดับกลาง-ลึกปรับตัวเข้ากับกายวิภาคของสุนัขและแมวได้อย่างแม่นยำ ปราศจากปัญหา Catastrophic Forgetting
    \end{itemize}
\end{enumerate}
\end{ansbox}
...
\begin{enumerate}[leftmargin=18pt, itemsep=3pt]
    ...
    \begin{itemize}
        \item \textbf{Model 4 (ตัด Data Augmentation ออก)}: ค่า Training Accuracy พุ่งสูงถึง \textbf{99.10\%} แต่ Validation Accuracy ทำได้เพียง \textbf{95.12\%} ส่งผลให้เกิด \textbf{Generalization Gap กว้างถึง +3.98\%} ซึ่งเป็นลักษณะบ่งชี้ของภาวะ Overfitting อย่างรุนแรง
        \item \textbf{Model 3 (เปิดใช้งาน Dynamic Augmentation)}: ค่า Training Accuracy อยู่ที่ \textbf{97.23\%} และ Validation Accuracy อยู่ที่ \textbf{97.47\%} ส่งผลให้ Generalization Gap ลดลงเหลือเพียง \textbf{+0.24\%} (Overfitting ถูกกำจัดอย่างสมบูรณ์)
    \end{itemize}
    \item \textbf{กลไกการทำงานทางเรขาคณิต (Affine Invariance Mechanism)}:
    \begin{itemize}
        \item การสุ่มแปลงภาพแบบพลวัตในแต่ละ Mini-batch (การหมุน $\pm 20^\circ$, การเลื่อนขนาน 10\%, การซูม 20\% และการพลิกภาพแนวนอน) ขยายขนาดตัวอย่างประสิทธิผล ($N_{eff} \gg N$) ทำให้โมเดลไม่เคยพบภาพที่ซ้ำเดิมในแต่ละ Epoch
...
\vspace{0.4em}
\textbf{คำตอบ (ANSWER)}:
จากการสร้างและทดสอบระบบจำลองสตรีมมิ่งภาพวิดีโอสด (Continuous Video Streaming Pipeline) ด้วยคลาส \texttt{DogCatStreamClassifier} ได้ผลการวิเคราะห์งบประมาณเวลาดังนี้:
...
\begin{enumerate}[leftmargin=18pt, itemsep=3pt]
    ...
    \begin{itemize}
        ...
        \item \textbf{3. MobileNetV2 Forward Pass}: สกัดฟีเจอร์และคำนวณผลทำนายใช้เวลา \textbf{18.4 ms} (เทียบเท่า Throughput $54.3\text{ FPS}$)
        \item \textbf{4. Temporal Smoothing (EMA)}: ปรับความเรียบของความน่าจะเป็น ($\hat{y}_t = 0.70 y_t + 0.30 \hat{y}_{t-1}$) ป้องกันการสั่นไหว ใช้เวลา \textbf{0.8 ms}
    ...
    โมเดล MobileNetV2 สามารถแปลงเป็น TensorFlow Lite (TFLite) ร่วมกับการบีบอัดแบบ Post-Training INT8 Quantization ซึ่งจะลดขนาดไฟล์โมเดลจาก 9.8 MB เหลือเพียง \textbf{2.6 MB (ลดลง 73.5\%)} และสามารถประมวลผลบนหน่วยประมวลผล NPU ของสมาร์ตโฟนได้ด้วยเวลาต่ำกว่า 8 ms ($> 120\text{ FPS}$) ทำงานแบบ On-Device โดยสมบูรณ์โดยไม่ต้องพึ่งพาคลาวด์
\end{enumerate}
\end{ansbox}
