...
เพื่อตอบสนองโจทย์ทางธุรกิจในการนำโมเดลไปพัฒนาต่อยอดสู่แอปพลิเคชัน AR Video Filter เราได้ทำการจำลองงบประมาณเวลาเชิงสถาปัตยกรรม (Architectural Latency Budget Simulation) ภายใต้สมมติฐานวิดีโอต่อเนื่อง 30 FPS ร่วมกับการปรับความเรียบเชิงเวลา (Temporal Exponential Smoothing) ดังแสดงในรูปที่ \ref{fig:plot_7}:
...
จากรูปที่ \ref{fig:plot_7}(a)--(b) การวิเคราะห์งบประมาณเวลาประมวลผลของท่อส่งข้อมูลเชิงสถาปัตยกรรมมีรายละเอียดดังนี้:
\begin{itemize}[leftmargin=1.8em, itemsep=0.5pt, parsep=0pt]
    \item \textbf{เวลาประมวลผลของโมเดลที่วัดได้จริง (Measured Offline Inference Pass):} MobileNetV2 Forward Pass บนสถาปัตยกรรม Apple Silicon macOS arm64 (Metal/MPS Hardware Acceleration) วัดจากการรันภาพทดสอบต่อเนื่องแบบ Offline ($N=170$ ภาพ) ได้ค่าเฉลี่ย \textbf{18.4 ms} ต่อภาพ (มัธยฐาน 18.2 ms, P95 20.8 ms, Throughput $54.3\text{ FPS}$) ซึ่งมีส่วนเผื่อเวลาด้านความปลอดภัยต่ำกว่าเพดาน 33.33 ms อยู่ถึง \textbf{14.93 ms}
    \item \textbf{การจัดสรรงบประมาณเวลาส่วนประกอบจำลอง (Simulated Engineering Budget Allowances):} สำหรับระบบ AR เชิงโต้ตอบ ได้จำลองกรอบเวลาออกแบบทางวิศวกรรม (Engineering Design Allowances) สำหรับโมดูลอื่น ๆ ได้แก่ Frame Capture \& Downsample (3.2 ms), Tensor Prep \& Normalization (1.1 ms), ส่วนอนุมานโมเดล MobileNetV2 (18.4 ms วัดจริง), Softmax \& EMA Smoothing (0.8 ms), และ Graphic Overlay (1.5 ms)
...
\vspace{0.25em}
\textbf{คำตอบ (ANSWER)}:
จากการวิเคราะห์งบประมาณเวลาเชิงสถาปัตยกรรม (Architectural Latency Budget Simulation) อ้างอิงจากผลการทดสอบการอนุมานแบบออฟไลน์ (Offline Sequential Inference) ของโมเดล MobileNetV2 (Phase 2) บนสถาปัตยกรรม Apple Silicon macOS arm64 (MPS Acceleration) ร่วมกับกรอบงบประมาณเวลาจำลองเชิงวิศวกรรม ได้ข้อสรุปดังนี้:
...
\begin{enumerate}[leftmargin=14pt, itemsep=1.5pt, topsep=1pt, parsep=0pt]
    ...
    \item \textbf{การแจกแจงงบประมาณเวลาประมวลผลเชิงสถาปัตยกรรม (Architectural Latency Breakdown)}:
    \begin{itemize}[leftmargin=10pt, itemsep=0.5pt, topsep=0.5pt, parsep=0pt]
...
\begin{itemize}[leftmargin=1.8em, itemsep=1pt]
    ...
    \item \textbf{Post-Training Quantization (INT8):} เมื่อทำ Quantization ลดทอนค่าน้ำหนักจาก Float32 เป็น Int8 จะช่วยลดขนาดไฟล์โมเดลลงได้สูงสุด $\approx 70\%$ และเพิ่มประสิทธิภาพการประมวลผลบนหน่วยประมวลผล NPU ของสมาร์ตโฟน (เช่น Apple Neural Engine หรือ Snapdragon NPU) สำหรับการทำงาน On-Device อย่างมีประสิทธิภาพ
\end{itemize}
