...
\begin{figure}[H]
    ...
    \caption{การจำลองงบประมาณเวลาเชิงสถาปัตยกรรมสำหรับระบบสตรีมมิ่งวิดีโอ: (a) เส้นเวลาความน่าจะเป็นและการสลับโหมดฟิลเตอร์ด้วย Temporal Exponential Smoothing ($\alpha = 0.70$) (b) การแจกแจงงบประมาณเวลาประมวลผล (Latency Breakdown Simulation) รวม 25.0 ms/เฟรม เทียบกับขีดจำกัด 33.33 ms}
    \label{fig:plot_7}
\end{figure}

จากรูปที่ \ref{fig:plot_7}(a)--(b) การวิเคราะห์งบประมาณเวลาประมวลผลของท่อส่งข้อมูลเชิงสถาปัตยกรรมมีรายละเอียดดังนี้:
\begin{itemize}[leftmargin=1.8em, itemsep=1pt]
    \item \textbf{เวลาประมวลผลของโมเดลที่วัดได้จริง (Measured Inference Pass):} MobileNetV2 Forward Pass บน GPU ใช้เวลาเฉลี่ย \textbf{18.4 ms} ต่อภาพ ($N=170$ ภาพทดสอบ, มัธยฐาน 18.2 ms, P95 20.8 ms, Throughput $54.3\text{ FPS}$) ซึ่งมีส่วนเผื่อเวลาด้านความปลอดภัยต่ำกว่าเพดาน 33.33 ms อยู่ถึง \textbf{14.93 ms}
    \item \textbf{การจัดสรรงบประมาณเวลาส่วนประกอบจำลอง (Simulated Component Budgets):} สำหรับระบบ AR แบบครบวงจร ได้จัดสรรเวลาตามมาตรฐานอุตสาหกรรม ได้แก่ Frame Capture \& Downsample (3.2 ms), Tensor Prep \& Normalization (1.1 ms), โมเดล MobileNetV2 (18.4 ms วัดจริง), Softmax \& EMA Smoothing (0.8 ms), และ Graphic Overlay (1.5 ms)
    \item \textbf{เวลารวมและข้อสรุปความเป็นไปได้ (Architectural Feasibility):} เวลารวมตามงบประมาณอยู่ที่ \textbf{25.0 ms ต่อเฟรม} (เทียบเท่า Throughput $40.0\text{ FPS}$) มี Safety Margin สูงถึง \textbf{8.33 ms (25.0\%)} ต่ำกว่าเพดาน 33.33 ms ยืนยันความเป็นไปได้ในเชิงสถาปัตยกรรม ทั้งนี้เป็นการประเมินเชิงงบประมาณเวลาจากการทำ Offline Inference โดยไม่ได้ต่อกล้องเว็บแคมหรือเรนเดอร์กราฟิกสด
\end{itemize}
