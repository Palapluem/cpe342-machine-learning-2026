...
\begin{enumerate}[leftmargin=18pt, itemsep=2pt]
    ...
    \end{itemize}
    \item เพื่อวิเคราะห์และแก้ไขปัญหา \textbf{ภาวะความไม่สมดุลของคลาสข้อมูล (Class Imbalance)} ในสัดส่วน 9:1 ผ่านการกำหนดค่าน้ำหนักชดเชยต้นทุนความผิดพลาด (\codevar{class\_weight='balanced'}) และการใช้ตัวชี้วัดประสิทธิภาพเฉพาะทาง
...
ต้นไม้ตัดสินใจเดี่ยวที่ปล่อยให้เติบโตอย่างไร้การควบคุมจะขยายกิ่งก้านจนกระทั่งทุกโหนดปลายใบมีความคลาดเคลื่อนเป็นศูนย์ ซึ่งนำไปสู่ภาวะ \textbf{การเรียนรู้เกินและความแปรปรวนสูง (Overfitting \& High Variance)} การควบคุมทำได้โดยกำหนดความลึกสูงสุด (\codevar{max\_depth}), จำนวนตัวอย่างขั้นต่ำในการแตกกิ่ง (\codevar{min\_samples\_split}) และการตัดแต่งกิ่งตามต้นทุนความซับซ้อน (Cost-Complexity Pruning: CCP):
\begin{equation}
...
ชุดข้อมูล \codevar{MBA.csv} มีขนาด $6,194$ แถว และ $10$ คอลัมน์ โดยมีกระบวนการจัดเตรียมตัวแปรดังนี้:
\begin{itemize}[leftmargin=18pt, itemsep=2pt]
    \item \textbf{การคัดกรองแถวข้อมูลเป้าหมาย}: คัดเลือกเฉพาะ $1,000$ แถวที่มีผลการตัดสิน \codevar{admission} (`Admit' = 900 ราย, `Waitlist' = 100 ราย)
    ...
    \item \textbf{การตัดตัวแปรระบุตัวตน}: ลบคอลัมน์ \codevar{application\_id} เนื่องจากเป็นเพียงลำดับไอดีที่ไม่มีคุณค่าเชิงทำนาย
    \item \textbf{การจัดการค่าสูญหาย}: ตัวแปร \codevar{race} ที่มีค่าสูญหาย ถูกกำหนดให้เป็นหมวดหมู่เฉพาะ `Missing'
    \item \textbf{การแปลงคุณลักษณะหมวดหมู่}: ตัวแปร \codevar{gender}, \codevar{international}, \codevar{major}, \codevar{race}, \codevar{work\_industry} ถูกแปลงเป็นรหัสหมวดหมู่จำนวนเต็ม (Category Codes)
...
โครงสร้างและแผนผังกฎการตัดสินใจของต้นไม้ตัดสินใจเดี่ยวที่ผ่านการตัดแต่งกิ่ง (\codevar{max\_depth=3}, \codevar{class\_weight='balanced'}) แสดงใน Figure 2
...
\noindent
\textbf{การแปลผล Figure 2}: โหนดราก (Root Node) ทำการตัดแบ่งด้วยตัวแปร \codevar{work\_industry} ($\le 12.5$) และ \codevar{gpa} ($\le 3.56$) ซึ่งชี้ให้เห็นว่าสายงานธุรกิจที่สังกัดและผลการเรียนระดับปริญญาตรีเป็นเกณฑ์ด่านแรกในการคัดกรองผู้สมัคร
...
\noindent
\textbf{การแปลผล Figure 3}: ตัวแปร 4 อันดับแรกที่มีอิทธิพลสูงสุดต่อผลการตัดสินใจคือ \textbf{GMAT Score}, \textbf{GPA}, \textbf{Work Experience} และ \textbf{Work Industry} ซึ่งมีสัดส่วนความสำคัญรวมกันมากกว่า $80\%$ ในทุกโมเดล ขณะที่ตัวแปรทางประชากรศาสตร์ (\codevar{gender}, \codevar{race}, \codevar{international}) มีบทบาทน้อยมาก
...
\begin{figure}[H]
    ...
    \caption{\textbf{Hyperparameter Validation Curves \& Bias-Variance Dynamics}: ซ้าย: ผลกระทบของความลึกต้นไม้ (\codevar{max\_depth}) ต่อ Decision Tree ขวา: ผลกระทบของจำนวนต้นไม้ (\codevar{n\_estimators}) ต่อ Random Forest}
\end{figure}
...
การทดลองบนชุดข้อมูล \codevar{MBA.csv} ยืนยันถึงคุณลักษณะเด่นและข้อจำกัดของแบบจำลองทั้ง 3 ประเภทตามทฤษฎีใน Lecture 5 โดย \textbf{Random Forest} เป็นแบบจำลองที่มีความเสถียรและให้ประสิทธิภาพในการจำแนกกลุ่มภาพรวมสูงสุด ($\text{CV AUC} = 0.6237$) ขณะที่ \textbf{Decision Tree} มีความโดดเด่นในด้านความสามารถในการอธิบายผล และ \textbf{Gradient Boosting} ให้ค่าความแม่นยำของกลุ่มหลักสูงสุด ($89.50\%$)
