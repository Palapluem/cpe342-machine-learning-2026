...
\begin{enumerate}[leftmargin=18pt, itemsep=2pt]
    ...
    \item เพื่อเปรียบเทียบวิถีการอยู่รอดและความเสี่ยงในการยกเลิกบริการระหว่างช่องทางการชำระเงินทั้ง 4 รูปแบบ (\texttt{PaymentMethod}): \textbf{Electronic check}, \textbf{Mailed check}, \textbf{Bank transfer (automatic)} และ \textbf{Credit card (automatic)}
...
ชุดข้อมูล \texttt{Telco-Churn.csv} มีโครงสร้างขนาด $7,043$ แถว และ $21$ คอลัมน์ โดยประกอบด้วยตัวแปรที่นำมาใช้ในการวิเคราะห์ดังนี้:
\begin{itemize}[leftmargin=18pt, itemsep=2pt]
    \item \textbf{Duration Variable ($T$)}: คอลัมน์ \texttt{tenure} แทนระยะเวลา (เดือน) ที่ลูกค้าผูกพันกับผู้ให้บริการ ($0 \le T \le 72$)
    \item \textbf{Event Indicator ($E$)}: คอลัมน์ \texttt{Churn} แปลงเป็น $E = 1$ เมื่อ Churn = `Yes' (เกิดเหตุการณ์) และ $E = 0$ เมื่อ Churn = `No' (ถูก Right-Censored)
    \item \textbf{Segmentation Covariate}: คอลัมน์ \texttt{PaymentMethod} แบ่งเป็น 4 กลุ่ม ได้แก่ Electronic check, Mailed check, Bank transfer (automatic) และ Credit card (automatic)
\end{itemize}
